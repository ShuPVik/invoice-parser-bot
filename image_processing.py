...
from utils import convert_image_to_base64, get_QR
...
async def handle_image(message, user_id, is_document, bot):
    ...
    try:
            ...
            if invoice == "Номер накладной отсутствует":
                try:
                    if error:
                        ...
                        logger.warning(
                            f"""Это накладная, но номер не удалось 
                            распознать для пользователя {user_id}."""
                        )
                        await bot.send_message(
                            user_id, "Не удалось распознать номер накладной."
                        )
                    return

                except TelegramForbiddenError:
                    logger.error(
                        f"""Бот не может отправить сообщение 
                        пользователю {user_id}. Возможно, он заблокирован."""
                    )
                except Exception as e:
                    logger.exception(
        ...
        logger.info(
            f"""Изображение сохранено: {image_id} для пользователя {user_id} с 
            накладной {invoice} с message_id: {images[image_id]["message_id"]}."""
        )
