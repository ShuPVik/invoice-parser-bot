        ...
        if image_data:
            ...
            if response.get("status") == "ok":
                try:
                    ...
                    logger.info(
                        f"""Буфер изображения готов, размер:
                          {image_buffer.getbuffer().nbytes} байт"""
                    )
                    ...
                    logger.info(f"user_id: {user_id}, info_chat_ids: {info_chat_ids}")
                    if user_id not in info_chat_ids:
                                ...
                                inline_keyboard=[
                                    [
                                        InlineKeyboardButton(
                                            text="Забрана",
                                            callback_data=f"received:{image_id}",
                                        )
                                    ],
                                    [
                                        InlineKeyboardButton(
                                            text="Доставлено",
                                            callback_data=f"delivered:{image_id}",
                                        )
                                    ],
                                    [
                                        InlineKeyboardButton(
                                            text="Прочее",
                                            callback_data=f"other:{image_id}",
                                        )
                                    ],
                                ]
                            ),
                        ...
                        logger.info(
                            ...
                            пользователю {user_id} с выбором действий."""
                        )
                        logger.info(
                            f"""Отправлено сообщение c message_id: 
                        ...
                        result = await invoice_processing(
                            invoice,
                        ...
                        if result is None:
                            logger.warning("invoice_processing вернул None.")
                            text = "Не удалось обработать накладную."
                        else:
                            text, _ = result
