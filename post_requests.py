    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                url_check_number, data=json.dumps(payload), headers=headers
            ) as response:
                logger.info(
        ...
        logger.exception(f"[CHECK_NUMBER] Ошибка при выполнении POST запроса: {e}")
        return {"error": str(e)}
...
async def post_request(qr_data, s3_file_key, status, headers):
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.post(
                url_sent_data, data=json.dumps(payload), headers=headers
            ) as response:
                logger.info(f"[SENT_DATA] Получен ответ со статусом {response.status}")
            ...
            logger.exception(f"[SENT_DATA] Ошибка при выполнении POST запроса: {e}")
            return {"error": str(e)}
